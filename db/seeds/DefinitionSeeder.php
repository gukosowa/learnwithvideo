...
class DefinitionSeeder extends AbstractSeed
{
    ...
    public function run()
    {
        ...
        $count = count($this->query('SELECT * FROM definition')->fetchAll());
        if($count != 0)
        {
            $this->emptyTable = false;
        } else {
            echo "\n\nCreate Definition DB\n";
    ...
    function insertTable($json, $cat) {
        foreach($json as $word) {
            ...
            if ($this->emptyTable) {
                ...
                $this->table('definition')->insert($data)->save();
            }
